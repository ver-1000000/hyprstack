...
`hyprstack` は window の順番と focus/swap 操作だけを扱います。 window を floating にするか、どの大きさで表示するかは Hyprland config 側の責務です。

floating stack として使う場合、通常の application window に `maximize` や `fullscreen` の window rule を使うことは避けるのが安全です。 maximized/fullscreen window は通常の floating z-order と別扱いになり、stack focus しても前面に来ないように見える場合があります。

大きい floating window として扱いたい場合は、`maximize` ではなく `float`、`center`、`size` を使ってください。
