...
`hyprstack` only handles window order and focus/swap actions. Whether windows float, how large they are, and which apps get special handling are Hyprland config responsibilities.

For floating stack workflows, avoid `maximize` or `fullscreen` window rules for normal app windows. Maximized/fullscreen windows may be handled outside the normal floating z-order, so stack focus can appear not to bring them to the front.

For large floating windows, prefer `float`, `center`, and `size` instead of `maximize`.
